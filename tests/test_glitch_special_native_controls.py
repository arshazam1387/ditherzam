...
DEFAULTS = {
    ...
    "Modulated Diffuse X": (1, 100, 0, 0, 0), "Uniform Modulation Y": (2, 50, 25, 100, 10, 100),
    "Uniform Modulation X": (1, 35, 20, 100, 8, 100), "Waveform": (1, 5, 30, 128, 0),
...
VARIANTS = {
    ...
    "Modulated Diffuse X": (4, 160, 30, 40, 30), "Uniform Modulation Y": (4, 60, 50, 150, 30, 400),
    "Uniform Modulation X": (4, 60, 50, 150, 30, 400), "Waveform": (8, 14, 70, 70, 80),
...
def test_uniform_modulation_y_has_six_native_controls_and_spacing_changes_pixels(image):
    entry = registry.get_entry("Uniform Modulation Y")
    assert entry.param_sliders == (
        "dither_parameter_slider", "smoothing_factor_slider",
        "bleed_fraction_slider", "diffusion_error_gain_slider",
        "diffusion_decay_slider", "diffusion_line_spacing_slider",
    )
    default = DEFAULTS["Uniform Modulation Y"]
    spread = default[:-1] + (400,)
    assert np.any(entry.func(image.copy(), default, 128.0) !=
                  entry.func(image.copy(), spread, 128.0))
