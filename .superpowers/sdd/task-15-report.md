...
- `save_raster` now accepts only non-empty HxW, HxWx3, or HxWx4 numeric
  uint8-like arrays and only PNG/JPEG extensions.
...
- JIT-off focused: `27 passed` for `tests/test_export_raster.py` and
  `tests/test_mask_render_integration.py`.
- Real-JIT focused: `45 passed` for `tests/test_export_raster.py` and
