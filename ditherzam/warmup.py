...
def warmup_render(registry=None, styles=DEFAULT_WARMUP_STYLES) -> None:
    ...
    engine = ColorEngine(builtin_palettes()["gameboy"], "nearest")

    # with a palette -> also compiles the nearest_indices njit kernel
    pipe = RenderPipeline(reg, engine, None)
    for style in styles:
...
def start_warmup_thread(registry=None, styles=DEFAULT_WARMUP_STYLES):
    ...
    t = threading.Thread(
        target=lambda: _safe(warmup_render, registry, styles),
